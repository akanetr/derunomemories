tyrano.plugin.kag.tag.ruby.start = function(pm) {
	...
	if ( text.length >= 1 ) {
		...
		if ( isVertical != "true" ) {
			...
			str = "</rt></ruby><span style='position: relative; display: inline;'><span class='custom_ruby' style='position: absolute; transform: translate(" + ruby_x + "px," + ruby_y + "px) scale(" + ruby_scale + "); left:" + start + "; right:" + end + ";";
		} else {
			//縦書き時
			ruby_y = parseFloat(parseFloat(ruby_y) - parseFloat(defaultPitch));
			str = "</rt></ruby><span style='position: relative ; writing-mode: initial; -webkit-writing-mode: initial; width: 1em; display: inline;'><span class='custom_ruby_rl' style='position: absolute; transform: translate(" + ruby_x + "px," + ruby_y + "px) scale(" + ruby_scale + "); top:" + start + "; bottom:" + end + ";";
		}
		...
		//divだとロード時におかしくなって、rubyだとsafariで表示がおかしくなるからspanを使用
		str += "'>" +  text + "</span></span>";
